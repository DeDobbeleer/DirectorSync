...
# lp_tenant_importer_v2/main.py (excerpt — add a new CLI sub-command)
"""Main CLI entrypoint.

Below is the routing-policies command block to add, mirroring `import-repos`.
Keep the surrounding structure of your existing main.py intact and insert this
subparser near the other importer commands.
"""

# --- inside `build_parser()` where other subcommands are defined ---
# rp_parser = subparsers.add_parser("import-routing-policies", help="Import Routing Policies from XLSX")
# rp_parser.add_argument("--tenant", required=True, help="Tenant key from tenants.yml")
# rp_parser.add_argument("--tenants-file", required=True, help="Path to tenants.yml")
# rp_parser.add_argument("--xlsx", required=True, help="Path to workbook")
# rp_parser.add_argument("--format", default="table", choices=["table", "json"], help="Output format")
# rp_parser.add_argument("--dry-run", action="store_true", help="Plan only, do not call Director")
# rp_parser.add_argument("--no-verify", action="store_true", help="Do not enforce pre-flight verifications")
# rp_parser.set_defaults(func=cmd_import_routing_policies)


# --- add the command handler alongside cmd_import_repos ---
# def cmd_import_routing_policies(args):
#     from lp_tenant_importer_v2.importers.routing_policies import RoutingPoliciesImporter
#     from lp_tenant_importer_v2.core.config import load_tenant_and_nodes_from_args
#     from lp_tenant_importer_v2.core.director_client import DirectorClient, ClientOptions
#     from lp_tenant_importer_v2.utils.reporting import print_rows
#
#     tenant, pool_uuid, nodes = load_tenant_and_nodes_from_args(args)
#     client = DirectorClient.from_env(ClientOptions(verify=not args.no_verify))
#
#     importer = RoutingPoliciesImporter()
#     rows = importer.run_for_nodes(
#         client=client,
#         pool_uuid=pool_uuid,
#         nodes=nodes,
#         xlsx_path=args.xlsx,
...
#     )
#     print_rows(rows, fmt=args.format)


# lp_tenant_importer_v2/resources/profiles.yml (append this block)
"""
RoutingPolicy:
  resource: "RoutingPolicies"
  xlsx:
    sheet: "RoutingPolicy"
    columns:
      cleaned_policy_name: { required: true }
      active:              { required: false, default: "true" }
      catch_all:           { required: false, default: "" }
