...
# --- NEW: simple fallback name matcher (same logic as str.contains) -----
def match_tenant_by_name(name: str, tenant_list: list[str]) -> str | None:
    low = (name or "").lower()
    # iterate by decreasing length to avoid collisions like "es" before "esrin"
    for t in sorted(tenant_list, key=lambda x: len(x), reverse=True):
        if t.lower() in low:
            return t
    return None

# === ENTITY EXPORTS — unchanged from original (copy) ===
...
def extract_device_groups(sync_data: dict) -> pd.DataFrame:
    """Extract all DeviceGroups (multitenant, unfiltered)."""
...
# --- NEW: load full config (for collector_to_tenant, etc.) ------
...
def split_entities_by_tenant(entities: dict[str, pd.DataFrame],
                             ...
                             config_dict: dict,
                             alerts_df=None,
                             user_list_df=None) -> None:  # <= NEW param, default None
    ...
    for tenant in tenant_list:
        ...
        with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:  # same engine as original
            for entity_name, df in entities.items():
                # Fully exported entities (unchanged)
                ...
                # Device / DeviceFetcher filtered via collector-first mapping
                ...
                # Other entities: name matching (unchanged)
...
def main():
    ...
    # Setup optional file logging (unchanged)
    if args.log_file:
    ...
    # <= updated signature (full config + alerts_df passed)
