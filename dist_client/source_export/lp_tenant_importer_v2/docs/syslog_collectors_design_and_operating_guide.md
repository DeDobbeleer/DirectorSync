...
**Concurrency**: allowed **within** a phase in the design; the current implementation processes rows sequentially without explicit phase barriers.
...
> **Implementation note:** The current code runs all rows sequentially in a single loop and updates an in-memory proxy set. It does **not** implement the 3-phase barriers with re-inventory or the dry-run simulation of Phase-1 success described above. For `uses_proxy`, the code checks the proxy IP against already-processed `use_as_proxy` rows within the same workbook; it does not re-fetch the SIEM inventory between phases.
