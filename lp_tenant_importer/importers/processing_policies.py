...
def import_processing_policies_for_nodes(
    ...
    """Import processing policies from XLSX to specified SIEM nodes.
    ...
    try:
        # Read ProcessingPolicy sheet
        logger.debug("Reading ProcessingPolicy sheet from %s", xlsx_path)
        df = pd.read_excel(xlsx_path, sheet_name="ProcessingPolicy", skiprows=0)
        # Drop header row if it contains 'row1'
        if 'row1' in df.iloc[0].astype(str).str.lower().values:
            df = df.iloc[1:].reset_index(drop=True)
        if df.empty:
            logger.warning("ProcessingPolicy sheet is empty in %s", xlsx_path)
            results.append({
                "siem": "none",
                "node": "none",
                "name": "N/A",
        ...
        logger.debug("Found %d processing policies in XLSX", len(df))
        ...
        for policy in policies:
            cleaned_policy = {
                ...
                "norm_policy": str(policy.get("norm_policy", "")).strip() or None,
                "enrich_policy": str(policy.get("enrich_policy", "")).strip() or None,
                "routing_policy_id": str(policy.get("routing_policy_id", "")).strip() or None,
            }
            if not cleaned_policy["name"]:
                logger.warning("Skipping policy with empty name: %s", policy)
                continue

            logger.debug("Processing processing policy: %s", cleaned_policy["name"])
            ...
            for target in targets:
                ...
                for node in nodes.get(target, []):
                    ...
                    if dry_run:
                        ...
                        logger.info(
                            "Dry-run: Would import processing policy %s to %s (%s)",
                            cleaned_policy["name"],
                            node_name,
                            siem_id,
                        )
                        ...
                        try:
                            ...
                            endpoint = client.build_endpoint(pool_uuid, siem_id, "processing-policies")
                            ...
                            if existing:
                                ...
                                logger.info(
                                    "Updated processing policy %s on %s (%s)",
                                    cleaned_policy["name"],
                                    node_name,
                                    siem_id,
                                )
                                ...
                                logger.info(
                                    "Created processing policy %s on %s (%s)",
                                    cleaned_policy["name"],
                                    node_name,
                                    siem_id,
                                )
                            ...
                            any_error = True
                            logger.error(
                                "Failed to import processing policy %s to %s (%s): %s",
                                cleaned_policy["name"],
                                node_name,
                                siem_id,
                                e,
        ...
        if not results:
            logger.warning("No processing policies processed for %s", xlsx_path)
            results.append({
                "siem": "none",
                "node": "none",
                "name": "N/A",
        ...
        logger.error("Failed to parse ProcessingPolicy sheet in %s: %s", xlsx_path, e)
        raise ValueError(f"Failed to parse ProcessingPolicy sheet: {e}")
