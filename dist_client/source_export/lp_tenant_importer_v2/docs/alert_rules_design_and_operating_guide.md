...
> **Current implementation status:** This document was written as a full design. The code in `importers/alert_rules.py` currently implements only the **core MyRules create/update/NOOP/SKIP** flow. The following are **not yet implemented**: activate/deactivate, share/unshare/transferOwnership, and all notification endpoints (Email/Syslog/HTTP/SMS/SNMP/SSH). Sections describing those features are kept as API reference but are marked below.
>
...
* **Apply** the core rule in a stable, idempotent sequence.

> **Implementation note:** Steps 2–4 below (active state, sharing, notifications) are **not yet implemented** in `importers/alert_rules.py`. The current importer only creates/updates the core rule.

  1. Create/Update the **core rule**;
  2. *(Planned)* Sync **active state** (activate/deactivate);
  3. *(Planned)* Apply **sharing** (RBAC: groups/users);
  4. *(Planned)* Apply **notifications** by type (Email, Syslog, HTTP, SMS, SNMP, SSH).
...
* `name` (→ `searchname`)
* `settings.user` (→ `owner`; required by the API, resolved to a user ID)
* `settings.risk`
* `settings.repos` (list)
* `settings.aggregate`
...
* `settings.extra_config.query` (the current code reads this column; `settings.livesearch_data.query` is not used)
...
* `settings.active` (bool → *would be used* for activate/deactivate; **not implemented**)
* `settings.visible_to` (groups), `settings.visible_to_users` (users) (**not implemented**)
* `settings.notifications` (JSON array of typed notification objects) (**not implemented**)
* `tenant_scope` (drives fetch flavor: My/Shared/Vendor/Used — only **MyAlertRules** is implemented)
...
* **state**: `active` (bool → *would drive* Activate/Deactivate after core change; **not implemented**).
* **rbac**: `visible_to_groups[]`, `visible_to_users[]` → Share/Unshare (**not implemented**).
* **notifications**: list of typed objects (`type: email|syslog|http|sms|snmp|ssh`, plus type-specific fields) (**not implemented**).

**Canonical comparison subset** (diff keys in the current code):
`risk, repos_csv, aggregate, condition_option, condition_value, limit,
 timerange_day|hour|minute, searchname, search_interval_minute,
 apply_jinja_template, description, original_data, manageable_by_csv,
 attack_tag_csv, assigned_to_raw, metadata_json`.

> Only the **core subset** participates in planning; RBAC/notifications/state are **not** reconciled in the current code.
...
* Tolerates `settings.extra_config.query` for the query field (the current code does not read `settings.livesearch_data.query`).
...
and returns a map `{searchname → rule_obj_with_id}`.

> The following scopes are described in this document as API reference but are **not implemented**: `GET .../AlertRules`, `SharedAlertRules/fetch`, `VendorAlertRules/fetch`, `UsedAlertRules/fetch`, `UsedSharedAlertRules/fetch`.
...
2. **State** (**not implemented**)

   * `settings.active = true` → *(planned)* `POST .../AlertRules/{id}/activate`
   * `false` → *(planned)* `POST .../AlertRules/{id}/deactivate`

3. **Sharing / RBAC** (**not implemented**)

   * If `visible_to*` provided → *(planned)* `POST .../AlertRules/{id}/share` with `rbac_config`.
   * If no visibility desired → *(planned)* `POST .../AlertRules/{id}/unshare`.

4. **Notifications** (**not implemented**)

   * Email → *(planned)* `POST .../AlertRules/{id}/EmailNotification`
   * Syslog → *(planned)* `POST .../AlertRules/{id}/SyslogNotification`
   * HTTP → *(planned)* `POST .../AlertRules/{id}/HTTPNotification`
   * SMS → *(planned)* `POST .../AlertRules/{id}/SMSNotification`
   * SNMP → *(planned)* `POST .../AlertRules/{id}/SNMPNotification`
   * SSH → *(planned)* `POST .../AlertRules/{id}/SSHNotification`
...
**Post-apply** (not implemented in current code): `active` (state), `visible_to*` (share/unshare), `notifications[]` (per-type).
...
**Status**: Partially implemented. The core create/update flow is operational; activation, sharing, and notifications remain design items.
