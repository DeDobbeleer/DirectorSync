...
> **Current implementation status:** This document describes the scope actually implemented in `importers/alert_rules.py`.
>
> **Implemented:** MyRules fetch (`AlertRules/MyAlertRules/fetch`), core rule CREATE/UPDATE/NOOP/SKIP, owner/assigned_to resolution, `manageable_by` incident-group resolution, MITRE `attack_tag` resolution, repos/log_source expansion, timerange/query/description/throttling/context-template/metadata handling.
>
> **Not implemented:** Activation/deactivation convergence (`settings.active` is ignored), Shared/Vendor/Used* scopes, share/unshare/transferOwnership, notifications (Email/HTTP/SMS/SNMP/SSH/Syslog).
...
| `owner` | string | **Yes** | `settings.user` | **Dependency**: resolve to a valid user ID via the Director `Users` list or via `profiles.AlertRules.options.default_owner`; if missing → **SKIP**. |
| `risk` | string | **Yes** | `settings.risk` | Must be one of: `low / medium / high / critical`. |
| `repos` | array of string | **Yes** | `settings.repos` | List of repo names/paths. **Dependency**: expanded to `ip:port/repo` paths on tenant backends (or kept as-is if already an `IP:port/path`). |
...
| `query` | string | No | `settings.extra_config.query` | The current code reads only this column. |
...
| `apply_jinja_template` | "on" | No | `settings.apply_jinja_template` | Map truthy to `"on"`. |
...
> **Not implemented in the current code.** `settings.active` is read from the sheet but is **not** used to call `activate`/`deactivate`. Rules are created/updated in the state currently returned by the Director API. If activation convergence is required, it must be performed outside the importer (e.g., via the Director UI or a separate scripted step).
