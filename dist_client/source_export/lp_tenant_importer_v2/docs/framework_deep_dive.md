...
DirectorSync v2 is an importer tool that synchronizes configuration data into LogPoint Director via HTTP APIs. It consolidates cross-cutting logic into a **common trunk**. The goals are:
...
**Invariant:** End users interact through a stable CLI and spreadsheet format. All processing is handled by the v2 common trunk.
...
- **One specification per `source`**. All rows for the same `policy_name` + `source` are **unioned**:
